...
namespace STB
{
    internal unsafe partial class StbImage
    {
        ...
        public const int STBI_SCAN_LOAD = 0;
        public const int STBI_SCAN_TYPE = 1;
        public const int STBI_SCAN_HEADER = 2;

        public static float StbiH2LGammaI = 1.0f / 2.2f;
        public static float StbiH2LScaleI = 1.0f;
        public static int StbiUnpremultiplyOnLoad;
        public static int StbiDeIphoneFlag;
        public static uint[] MulTable = { 0, 0xff, 0x55, 0x49, 0x11, 0x21, 0x41, 0x81, 0x01 };
        public static uint[] ShiftTable = { 0, 0, 0, 1, 0, 2, 4, 6, 0 };
        ...
        public static void stbi_set_flip_vertically_on_load(int flagTrueIfShouldFlip)
        {
            StbiVerticallyFlipOnLoad = flagTrueIfShouldFlip;
        }
        ...
        public static void stbi__vertical_flip_slices(void* image, int w, int h, int z, int bytesPerPixel)
        {
            var slice = 0;
            var slice_size = w * h * bytesPerPixel;
            var bytes = (byte*)image;
            for (slice = 0; slice < z; ++slice)
            {
                stbi__vertical_flip(bytes, w, h, bytesPerPixel);
                bytes += slice_size;
            }
        }

        public static byte* stbi__load_and_postprocess_8bit(StbiContext s, int* x, int* y, int* comp, int reqComp)
        {
            var ri = new StbiResultInfo();
            var result = stbi__load_main(s, x, y, comp, reqComp, &ri, 8);
            if (result == null)
                return null;
            if (ri.bits_per_channel != 8)
            {
                result = stbi__convert_16_to_8((ushort*)result, *x, *y, reqComp == 0 ? *comp : reqComp);
                ri.bits_per_channel = 8;
            }

            if (StbiVerticallyFlipOnLoad != 0)
            {
        ...
        public static ushort* stbi__load_and_postprocess_16bit(StbiContext s, int* x, int* y, int* comp, int reqComp)
        {
            var ri = new StbiResultInfo();
            var result = stbi__load_main(s, x, y, comp, reqComp, &ri, 16);
            if (result == null)
                return null;
            if (ri.bits_per_channel != 16)
            {
                result = stbi__convert_8_to_16((byte*)result, *x, *y, reqComp == 0 ? *comp : reqComp);
        ...
        public static byte* stbi__convert_format(byte* data, int imgN, int reqComp, uint x, uint y)
        {
            var i = 0;
            var j = 0;
            byte* good;
            if (reqComp == imgN)
                return data;
            good = (byte*)stbi__malloc_mad3(reqComp, (int)x, (int)y, 0);
            if (good == null)
            {
                CRuntime.Free(data);
                return (byte*)(ulong)(stbi__err("outofmem") != 0 ? (byte*)null : null);
        ...
        public static void stbi_set_unpremultiply_on_load(int flagTrueIfShouldUnpremultiply)
        {
            StbiUnpremultiplyOnLoad = flagTrueIfShouldUnpremultiply;
        }

        public static void stbi_convert_iphone_png_to_rgb(int flagTrueIfShouldConvert)
        {
            StbiDeIphoneFlag = flagTrueIfShouldConvert;
        }
        ...
        public static int stbi__shiftsigned(uint v, int shift, int bits)
        {
            if (shift < 0)
                ...
                v >>= shift;
            v >>= 8 - bits;
            return (int)(v * (int)MulTable[bits]) >> (int)ShiftTable[bits];
        }
